...
- **代理人**：给锁定的 5 个 A 类重点客户上传外部小工具生成的 KYC 报告 PDF（系统内只记录客户编码，不录入真实姓名/联系方式）；双周提交经营动态（含新单件数/保费/FYC/转介绍数量、新增/跟进客户数）；查看教练反馈；查看集中辅导会总结、盘客报告；查看个人成长报告，报告页顶部有系统实时算的「五维数据总览」雷达图（客户经营能力/KYC应用能力/活动量达成/业绩转化能力/转介绍开发），仅供本人参考，不是项目经理的最终评分
- **教练**：查看学员经营动态，提交诊断反馈
...
3. **设置生产环境变量**（务必修改，不要使用默认值）
   ...
   ```
   成长报告/结业报告的「AI 生成草稿」功能会调用 MiniMax 的 Chat Completion 接口。传给 AI 的只有系统算好的结构化统计数字（新增/跟进客户数、KYC 报告上传数、经营动态提交率、教练反馈覆盖率、出勤率、新单件数/保费/FYC/转介绍数量），不会传任何客户原始信息、上传的 KYC PDF 内容，或经营动态/教练反馈的文字原文。AI 生成的内容始终是草稿，需要项目经理人工检查后再发布；如果对已发布内容重新生成，会自动打回草稿状态，不会静默覆盖已发布内容。

   成长报告是五维打分（客户经营能力/KYC应用能力/活动量达成/业绩转化能力/转介绍开发），全部五项现在都有自动建议分：「活动量达成」是出勤率与经营动态提交率的真实完成率；「KYC应用能力」是 5 个锁定 A 类客户的 KYC 报告上传数量占比，也是绝对完成率（上传的 PDF 内容本身不做质量评估，系统不解析 PDF 内容——只统计数量，避免把客户信息经由内容分析再传给外部模型）；「客户经营能力」「业绩转化能力」「转介绍开发」因为公司还没定统一的目标基准数字，系统给的是跟本期学员里最高值相比的相对分，不是对绝对目标的完成率——等定了具体目标，可以把 `compute_growth_score_suggestions()` 里的相对分改成目标完成率。
...
- **KYC 报告**：改为代理人用外部小工具生成 PDF 后手动上传，系统不再由代理人在站内填字段自动拼装生成——历史上自动生成的 KYC 文字报告（`kyc_reports`/`clients` 两张表）数据还留在数据库里，没有删除，只是应用层不再读写。
- **权限模型**：按 Role.txt 的五个角色一一对应实现。账号只有启用/禁用，没有硬删除。孤儿单客户的分配已改为线下完成，系统内不再提供录入/批量导入/编辑/删除孤儿单客户的入口；代理人的「自有存量客户」录入功能已整体下线，改为固定 5 个 A 类重点客户位的 KYC PDF 上传。
