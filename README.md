...
- **教练**：查看学员经营动态，提交诊断反馈
- **项目经理**：孤儿单客户分配、开启经营动态周期、录入集中辅导会/盘客报告（含出勤）、录入沙龙 1&1 陪谈、录入代理人成长报告、录入结业报告
- **保司内勤**：以上全部内容的只读视图
...
- **登录鉴权**：账号密码用 `werkzeug.security` 做加盐哈希存储，会话用 Flask 签名 Cookie；没有做限流/验证码，没有实现「登录双重验证」的真实二次验证流程（系统设置里的开关目前只是配置项占位）。生产使用前建议至少加一层登录失败限流。
- **KYC 报告**：由代理人录入的字段（年龄区间、家庭结构、收入区间、已有保障等）按规则模板拼装生成，是陪跑项目内部的盘客参考，**不是核保结论**，已在报告末尾注明。
- **权限模型**：按 Role.txt 的五个角色一一对应实现，尚未做「删除数据」类操作（只有启用/禁用账号，没有硬删除客户/报告），避免误删导致数据不可恢复。
