...
def build_growth_prompt(agent_name, metrics):
    lines = [
        ...
        "",
        "数据如下：",
        f"- 双周经营动态累计新增/跟进客户：{metrics['client_activity']} 人次",
        f"- 已上传 KYC 报告的重点客户数：{metrics['kyc_uploaded']}/5",
...
def build_final_report_prompt(summary):
    lines = [
        ...
        f"- 人均双周经营动态累计新增/跟进客户：{summary['avg_client_activity']:.1f} 人次",
        f"- 人均已上传 KYC 报告数：{summary['avg_kyc_uploaded']:.1f}/5",
